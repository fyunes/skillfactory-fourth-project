...
const ItemDetail = ({ id }) => {
  ...
  const getProduct = async (id) => {
    ...
    console.log(productSnap.data(), "hola");
    ...
    >
      ...
      >
        <Box w="50%" h="100%" borderRadius={5} >
          <Image objectFit="cover" borderRadius={5}  boxSize="100%" src={product.image} />
        </Box>
      </Box>      
      <Box width="30%" borderWidth={2} borderRadius={5} >
        ...
        <NumberInput mb={3} size="sm" maxW={20} defaultValue={1} min={1} max={product.stock}>
            <NumberInputField />
            <NumberInputStepper>
              <NumberIncrementStepper />
              <NumberDecrementStepper />
